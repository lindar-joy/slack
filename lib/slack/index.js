...
const repos = require('../actions/repos');
...
module.exports = (robot) => {
  ...
  router.use(/\/actions.*/, middleware.parseState);

  router.use('/repos', middleware.attachGitHubUserData);
  ...
  router.post('/repos', repos.get);
  router.post('/actions:interactive_message:actions:create-issue', actions.createIssue);

  router.post(/actions:interactive_message:.*:cancel/, unfurls.cancel);

  router.post('/actions:dialog_submission:create-issue-dialog', middleware.assertPermissions(create.issue.submit, { issues: 'write', pull_requests: 'write' }));
  router.post('/actions:dialog_submission:create-deployment-dialog', middleware.assertPermissions(create.deployment.submit, { deployments: 'write' }));
  ...
  router.post(
    '/options:dialog_suggestion:add-comment:selectedUrl',
    middleware.requireGitHubUser,
